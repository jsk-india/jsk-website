...
export function ApplicationForm({ jobId, jobTitle }: { jobId: string; jobTitle: string }) {
  ...
  if (state.success) {
    return (
      <div className="rounded-lg border-2 border-green-200 bg-green-50 p-8 text-center">
        ...
        <h3 className="mt-2 text-xl font-bold text-green-800">Application submitted!</h3>
  ...
  return (
    <form action={action} encType="multipart/form-data" className="space-y-6">
      ...
      <div className="grid gap-6 sm:grid-cols-2">
        <div>
          ...
          <input id="name" name="name" type="text" required
            className="mt-1 w-full rounded-md border border-surface-100 px-3 py-2 text-sm focus:border-brand-red focus:outline-none" />
        </div>
        <div>
      ...
      <div>
        <label htmlFor="resume" className="block text-sm font-medium">Resume / CV * <span className="text-xs text-ink-600">(PDF, DOC · max 5 MB)</span></label>
