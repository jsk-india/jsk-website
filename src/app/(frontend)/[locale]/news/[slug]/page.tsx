  ...
  return (
    <article className="mx-auto max-w-3xl px-4 py-16 sm:px-6">
      ...
      <p className="text-sm text-ink-600">
        {new Date(article.publishedAt as string).toLocaleDateString('en-IN', {
      ...
      {article.summary && (
        <p className="mt-4 text-lg text-ink-600">{article.summary}</p>
      )}
      ...
      {cover && (
        <div className="relative mt-8 aspect-[16/9] overflow-hidden rounded-lg bg-surface-100">
          <Image
            src={cover}
            alt={mediaAlt(article.cover) || (article.title as string)}
            fill
            priority
      ...
      {article.body ? (
        ...
        <p className="mt-10 text-ink-300">No content yet.</p>
      )}
    </article>
  )
}
